...
# Скрипт для виправлення nginx - ЗАЛИШИТИ /api префікс при проксуванні
# Бекенд очікує /api/auth/login, а не /auth/login
...
echo "🔧 Виправлення nginx конфігурації - залишити /api префікс..."
echo ""
...
echo "📝 Створення правильної конфігурації (З /api префіксом)..."
echo ""
...
server {
    ...
    # Backend API - ВИПРАВЛЕНО: залишаємо /api префікс
    location /api {
        # НЕ прибираємо /api префікс - бекенд очікує /api/auth/login
...
curl -s -X POST http://localhost/api/auth/login \
  ...
  -d '{"email":"admin@foryou.ae","password":"admin123"}' | head -5 || echo "   ⚠️  Не працює"
echo ""
